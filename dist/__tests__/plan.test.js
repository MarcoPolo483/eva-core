...
describe("Plan", () => {
    it("runs steps in order and summarizes", async () => {
        const plan = makePlan("p", [
            { name: "a", action: async () => ({ ok: true, value: {} }) },
            { name: "b", action: async () => ({ ok: true, value: {} }) }
        ]);
    ...
    it("stops on first failure", async () => {
        const plan = makePlan("p", [
            { name: "a", action: async () => ({ ok: false, error: new Error("x") }) },
            { name: "b", action: async () => ({ ok: true, value: {} }) }
        ]);
        const r = await runPlan(plan);
    ...
    it("skips step when predicate false", async () => {
        const plan = makePlan("p", [
            { name: "a", when: () => false, action: async () => ({ ok: true, value: {} }) }
        ]);
        const r = await runPlan(plan);
        expect(r.ok).toBe(true);
        if (r.ok)
            expect(r.value.steps[0].ok).toBe(true);
    });
});
