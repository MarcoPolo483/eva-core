 ...
 */
export const defaultConfigurations: Record<string, Partial<EvaCosmosEnvironmentConfig>> = {
  development: {
...
export class EvaCosmosConfigLoader {
  static loadFromEnvironment(environment: string = 'development'): EvaCosmosEnvironmentConfig {
    ...
    return {
      ...
      consistencyLevel: (process.env.COSMOS_CONSISTENCY_LEVEL as any) || baseConfig.consistencyLevel,
...
export class EvaCosmosClientFactory {
  private static clients: Map<string, EvaCosmosClient> = new Map();

  /**
   * Create or get cached client for environment
   */
  static async getClient(environment: string = 'development'): Promise<EvaCosmosClient> {
    ...
    const clientConfig: EvaCosmosConfig = {
   ...
   */
  static async createCustomClient(config: EvaCosmosConfig): Promise<EvaCosmosClient> {
    if (config.keyVaultUrl && !config.primaryKey) {
