...
export interface DocumentFilter {
  ...
  createdBefore?: Date;
  tags?: string[];
  search?: string; // Text search in filename or metadata
  hasVectors?: boolean;
}
...
export class EvaDocumentRepository {
  ...
  async createDocument(
    document: Omit<DocumentMetadata, 'id' | 'created_at' | 'updated_at' | 'tenantId'>,
  ...
  async markForReprocessing(
    ...
    for (const documentId of documentIds) {
      try {
        const updated = await this.updateProcessingStatus(
          documentId,
          'uploading',
          tenantContext,
          {
  ...
  async getProcessingProgress(
    ...
    const documents = await Promise.all(
      documentIds.map(id => this.getDocument(id, tenantContext))
    );
