...
import type { ChatModel, ToolSpec, ChatMessage } from '../spi/llm.js';
import type { KeyValue, BlobStore } from '../spi/storage.js';
import { NoopTelemetry } from '../spi/telemetry.js';
import type { EmbeddingProvider } from '../spi/embeddings.js';
import type { VectorStore, VectorQueryResult } from '../spi/vector-store.js';
...
describe('SPI Interfaces - ChatModel', () => {
  ...
  it('should handle tool calls in ChatResult', async () => {
    ...
    const messages: ChatMessage[] = [{ role: 'user', content: 'search for info' }];
    const tools: ToolSpec[] = [{ name: 'search', description: 'Search tool' }];
...
describe('SPI Interfaces - Storage', () => {
  it('should define KeyValue interface contract', async () => {
    ...
    const value = await mockKV.get<string>('key1');
...
describe('SPI Interfaces - VectorStore', () => {
  ...
  it('should handle optional filter in vector search', async () => {
    ...
    const results = await mockVectorStore.search(
      [0.5, 0.6, 0.7],
      10,
      { category: 'tech' }
    );
